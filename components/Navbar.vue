<template>
  <nav
    ...
    :class="[
      isScrolled
        ? 'bg-dj-black/90 backdrop-blur-md py-4'
        : 'bg-transparent py-6',
    ]"
  >
    <div class="container mx-auto px-6 flex items-center justify-between">
      ...
      <NuxtLink to="/" class="flex items-center gap-3 group no-underline">
        <img
          src="~/assets/css/img/dj_wonlo_no_text-removebg-preview.png"
          alt="DJ Wonlo Logo"
        ...
        >
          <span
            class="text-dj-primary group-hover:text-white transition-colors duration-300"
            >DJ</span
          >
          <span
            class="group-hover:text-dj-primary transition-colors duration-300"
          >
            Wonlo</span
          >
        </div>
      ...
      <div class="hidden md:flex items-center gap-8">
        <template v-for="link in links" :key="link.name">
          ...
          >
            {{ link.name }}
            <span
              class="absolute -bottom-1 left-0 w-0 h-0.5 bg-dj-primary transition-all duration-300 group-hover:w-full"
            ></span>
          </a>
          ...
          <NuxtLink
            ...
            :class="{
              'text-dj-primary': route.path === link.href && !link.isScroll,
            }"
          >
            {{ link.name }}
            <span
      ...
      <button
        @click="isOpen = !isOpen"
        class="md:hidden text-white hover:text-dj-primary transition-colors focus:outline-none"
      >
    ...
    >
      <div
        v-if="isOpen"
        class="md:hidden absolute top-full left-0 w-full bg-dj-black/95 backdrop-blur-xl border-b border-white/10 shadow-2xl"
      >
